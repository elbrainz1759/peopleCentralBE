  ...
  async findByStaff(staffId: number): Promise<LeaveBalance[]> {
    ...
    try {
      const [rows] = await conn.query<mysql.RowDataPacket[]>(
        `SELECT lb.*, lt.name AS leave_type_name, e.first_name, e.supervisor, e.last_name, e.designation, e.location, e.program, e.department, l.name AS location_name, d.name AS department_name, p.name AS program_name
